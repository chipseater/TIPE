...
let nv_generation taille_carte nb_villages =
  ...
  let position = gen_village_positions (taille_carte / taille_troncon) nb_villages in
  (carte, position)
...
let evolution_par_tour (village : village) (carte : carte) (test : bool ref) = 
  ...
  try
  let nv_logistique =
    manque_of_main_d_oeuvre temp_logistique (village.logistique) (village.position_list) carte village
  in  
...
let createvillage (arbre : arbre) (arbrepos:arbrepos) (pos : position) (carte : carte) (id : int) :
    village =
  (* init le village *)
  depart carte pos;
  {
...
let noter (village : village) (carte : carte) : int =
...
let fait_genertion arbre_tab arbrepos_tab carte_de_base pos_array : arbre array* arbrepos array * evaluation = 
  ...
  let run_arbre_at_pos i j = 
    ...
    let nv_village = createvillage arbre_tab.(j) arbrepos_tab.(j) pos_array.(i) carte j in
  ...
  let run_position i =
    ...
    try    
    (fun () ->
      parallel_for ~start:0 ~finish:(nb_arbres - 1) ~body:(run_arbre_at_pos i) generation_pool)
    |> run generation_pool
  ...
  try
  let (meilleur_arbres_array,meilleur_arbrespos_array) = selection score_mat arbre_tab arbrepos_tab in 
