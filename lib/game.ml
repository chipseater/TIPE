...
let selection score tree_tab treepos_tab =
  ...
  let arbres_tries = Array.make (nb_arbres / nombre_survivant) Vide in
  let arbrespos_tries = Array.make (nb_arbres/nombre_survivant) Nil in 
  for i = 0 to (nb_arbres / nombre_survivant) - 1 do
...
let game1 ?(nb_villages = 5) ?(nb_trees = 2) ?(taille_carte = 800) (n : int) =
