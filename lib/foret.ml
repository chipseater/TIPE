...
let const_cond t =
  match to_string (member "type" t) with
  | "InegaliteBrute" ->
      InegaliteBrute
        ( echange_ressource (to_string (member "ressource1" t)),
...
let const_act t =
  ( echange_arg (to_string (member "arg" t)),
    echange_bat (to_string (member "bat" t)),
    echange_prio (to_string (member "prio" t)) )

let rec construc (t : Safe.t) =
  Node
    ( const_cond (member "condition" t),
