...
let print_matrix_list list = 
  List.iter begin
    fun x -> 
      List.iter (fun y -> print_int y; print_char ' ') x;
      print_char '\n' end list
;;
...
let array_to_json_list to_json (array: 'a array) = 
  ...
  let rec listify index =
    if (index = n) then []
    else (to_json array.(index))::(listify (index + 1))
  in `List (listify 0)
;;
...
let serialize_map map =
  `List [matrix_to_json_list serialize_chunk map]
